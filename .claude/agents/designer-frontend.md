...
3. **Mapa** (/mapa): Brasil em Leaflet + OpenStreetMap, marcadores agrupados (cluster) de eventos; ao clicar: descrição do evento, vítimas ligadas, crimes documentados e a seção comparativa jurídica VINDA DA API (você apenas exibe; nunca redigir conteúdo jurídico ou histórico no frontend — todo texto factual vem do banco curado).
